...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "MyExp_Kv2like", _hoc_MyExp,
 "rates_Kv2like", _hoc_rates,
 "vtrap_Kv2like", _hoc_vtrap,
 0, 0
};
#define MyExp MyExp_Kv2like
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 Kv2like /mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/Kv2like.mod\n");
...
static int  rates ( _threadargsproto_ ) {
 ...
 _lqt = pow( 2.3 , ( ( celsius - 21.0 ) / 10.0 ) ) ;
   ...
   mBeta = 0.02 * MyExp ( _threadargscomma_ - ( v + 1.27 ) / 120.0 ) ;
   mInf = mAlpha / ( mAlpha + mBeta ) ;
   mTau = 2.5 * ( 1.0 / ( _lqt * ( mAlpha + mBeta ) ) ) ;
   hInf = 1.0 / ( 1.0 + MyExp ( _threadargscomma_ ( v + 58.0 ) / 11.0 ) ) ;
   h1Tau = ( 360.0 + ( 1010.0 + 23.7 * ( v + 54.0 ) ) * MyExp ( _threadargscomma_ - pow( ( ( v + 75.0 ) / 48.0 ) , 2.0 ) ) ) / _lqt ;
   h2Tau = ( 2350.0 + 1380.0 * MyExp ( _threadargscomma_ - 0.011 * v ) - 210.0 * MyExp ( _threadargscomma_ - 0.03 * v ) ) / _lqt ;
     return 0; }
...
double vtrap ( _threadargsprotocomma_ double _lx , double _ly ) {
  ...
  if ( fabs ( _lx / _ly ) < 1e-6 ) {
     ...
     _lvtrap = _lx / ( MyExp ( _threadargscomma_ _lx / _ly ) - 1.0 ) ;
     }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/Kv2like.mod";
static const char* nmodl_file_text = 
  ": Kv2-like channel\n"
