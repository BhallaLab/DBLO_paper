...
#define q10 q10_kd
 double q10 = 2.3;
#define vhalfh vhalfh_kd
 double vhalfh = -67;
#define vhalfm vhalfm_kd
 double vhalfm = -43;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kd /mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/kd.mod\n");
...
static int  trates (  double _lv ) {
   double _lqt , _lArg1 , _lArg2 ;
 _lqt = pow( q10 , ( ( celsius - 22.0 ) / 10.0 ) ) ;
   _lArg1 = ( _lv - vhalfm ) / km ;
   if ( _lArg1 < - 50.0 ) {
     minf = 0.0 ;
     }
   else if ( _lArg1 > 50.0 ) {
     ...
     minf = 1.0 - 1.0 / ( 1.0 + exp ( _lArg1 ) ) ;
     }
   _lArg2 = ( _lv - vhalfh ) / kh ;
   if ( _lArg2 < - 50.0 ) {
     hinf = 1.0 ;
     }
   else if ( _lArg2 > 50.0 ) {
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 45 in file kd.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/kd.mod";
static const char* nmodl_file_text = 
  "TITLE K-D\n"
