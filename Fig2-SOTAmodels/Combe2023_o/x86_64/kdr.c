...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kdr /mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/kdr.mod\n");
...
double alpn ( _threadargsprotocomma_ double _lv ) {
   double _lalpn;
 double _lArg ;
 _lArg = 1.e-3 * zetan * ( _lv - vhalfn ) * 9.648e4 / ( 8.315 * ( 273.16 + celsius ) ) ;
   if ( _lArg < - 50.0 ) {
     _lalpn = 0.0 ;
     }
   else if ( _lArg > 50.0 ) {
...
double betn ( _threadargsprotocomma_ double _lv ) {
   double _lbetn;
 double _lArg ;
 _lArg = 1.e-3 * zetan * gmn * ( _lv - 13.0 ) * 9.648e4 / ( 8.315 * ( 273.16 + celsius ) ) ;
   if ( _lArg < - 50.0 ) {
     _lbetn = 0.0 ;
     }
   else if ( _lArg > 50.0 ) {
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
   double _la , _lc , _lqt , _lArg ;
 _lqt = pow( q10 , ( ( celsius - 24.0 ) / 10.0 ) ) ;
   _la = alpn ( _threadargscomma_ _lv ) ;
   ninf = 1.0 / ( 1.0 + _la ) ;
   _lArg = - 339.44 * ( _lv - 13.0 ) / ( 8.315 * ( 273.16 + celsius ) ) ;
   if ( _lArg < - 50.0 ) {
     _lc = 0.0 ;
     }
   else if ( _lArg > 50.0 ) {
   ...
   taun = 0.7 * betn ( _threadargscomma_ _lv ) / ( _lqt * a0n * ( 1.0 + _lc ) ) ;
   if ( taun < nmax ) {
     taun = nmax ;
     }
    return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   tau = 0.7 * betn ( _threadargscomma_ v ) / ( 0.02 * ( 1.0 + exp ( - 339.44 * ( v - 13.0 ) / ( 8.315 * ( 273.16 + celsius ) ) ) ) ) ;
   gkdr = gkdrbar * n ;
   ik = gkdr * ( v - ek ) ;
   }
 _current += ik;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/kdr.mod";
static const char* nmodl_file_text = 
  "TITLE K-DR channel\n"
  ...
  "FUNCTION alpn(v(mV)) {\n"
  "	LOCAL Arg\n"
  "	Arg=1.e-3*zetan*(v-vhalfn)*9.648e4/(8.315*(273.16+celsius))\n"
  "	\n"
  "    if (Arg<-50) {alpn=0}\n"
  ...
  "FUNCTION betn(v(mV)) {\n"
  "	LOCAL Arg\n"
  "	Arg=1.e-3*zetan*gmn*(v-13)*9.648e4/(8.315*(273.16+celsius))\n"
  "\n"
  "    if (Arg<-50) {betn=0}\n"
  ...
  "        a = alpn(v)\n"
  "        ninf = 1/(1+a)\n"
  "		Arg=-339.44*(v-13)/(8.315*(273.16+celsius))\n"
  "		if (Arg<-50) {c=0}\n"
  "		else if (Arg>50) {c=exp(50)}\n"
  "		else {c=exp(Arg)} \n"
