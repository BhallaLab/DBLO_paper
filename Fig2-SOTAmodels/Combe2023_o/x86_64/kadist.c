...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kad /mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/kadist.mod\n");
...
double alpn (  double _lv ) {
 ...
 _lArg = ( _lv - tq ) / qq ;
   if ( _lArg < - 50.0 ) {
     ...
     _lzeta = zetan + pw / ( 1.0 + exp ( _lArg ) ) ;
     }
   _lArg = 1.e-3 * _lzeta * ( _lv - vhalfn ) * 9.648e4 / ( 8.315 * ( 273.16 + celsius ) ) ;
   if ( _lArg < - 50.0 ) {
     _lalpn = 0.0 ;
     }
   else if ( _lArg > 50.0 ) {
...
double betn (  double _lv ) {
 ...
 _lArg = ( _lv - tq ) / qq ;
   ...
   _lArg = 1.e-3 * _lzeta * gmn * ( _lv - vhalfn ) * 9.648e4 / ( 8.315 * ( 273.16 + celsius ) ) ;
   if ( _lArg < - 50.0 ) {
     _lbetn = 0.0 ;
     }
   else if ( _lArg > 50.0 ) {
...
double alpl (  double _lv ) {
   double _lalpl;
 double _lArg ;
 _lArg = 1.e-3 * zetal * ( _lv - vhalfl ) * 9.648e4 / ( 8.315 * ( 273.16 + celsius ) ) ;
   if ( _lArg < - 50.0 ) {
     _lalpl = 0.0 ;
     }
   else if ( _lArg > 50.0 ) {
...
double betl (  double _lv ) {
   double _lbetl;
 double _lArg ;
 _lArg = 1.e-3 * zetal * gml * ( _lv - vhalfl ) * 9.648e4 / ( 8.315 * ( 273.16 + celsius ) ) ;
   if ( _lArg < - 50.0 ) {
     _lbetl = 0.0 ;
     }
   else if ( _lArg > 50.0 ) {
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 56 in file kadist.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/mnt/h/Thesis work/Compilations/ExistingModels/Combe2023/267599/cholinergic_shift_generalize/kadist.mod";
static const char* nmodl_file_text = 
