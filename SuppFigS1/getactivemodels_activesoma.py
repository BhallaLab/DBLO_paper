...
Multiprocessthis_appendsave(ourfunc, range(50000), [], ['temp_activemodels.pkl'], seed=np.random.randint(1,100000), npool=100)
...
with open('temp_activemodels.pkl', "rb") as f, open('activemodels_activesoma.json', "a") as file:
    while True:
        try:
            model = pickle.load(f)
            if len(model) > 0:
