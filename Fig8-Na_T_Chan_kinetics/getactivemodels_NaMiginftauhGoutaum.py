    ...
    "Parameters": {
        ...
        "Channels": {
            "Na_Chan": {
                "Gbar": 2e-5,
                "Erev": 0.06,
                "Kinetics": "Na_Chan_MiginftauhGoutaum",
            },
            "K_DR_Chan": {
                "Gbar": 1e-6,
                "Erev": -0.100,
...
def ourfunc(i):
    ...
    # print('#################################################')
    basemodel["Parameters"]["Channels"]["Na_Chan"] = baseModel["Parameters"]["Channels"]["Na_Chan"]
    ...
    basemodel["Parameters"]["Channels"]["Na_Chan"]["Gbar"] = Na_Chan_Gbar
...
for cells in validcells:
    ...
    Multiprocessthis_appendsave(
       ourfunc, range(5000), [], ["tempactivemodels.pkl"], seed=seeed, npool=120
    )

    with open("tempactivemodels.pkl", "rb") as f, open('activemodels_NaMiginftauhGoutaum.json', "a") as file:
        while True:
            try:
                model = pickle.load(f)
                if len(model) > 0:
