...
f.write(f'Kruskal-Wallis H-test {GouwensvsRoyeckvsMigliore_kruskal}\n')
...
f.write(f'Dunn’s test {GouwensvsRoyeckvsMigliore_dunn}\n')
...
f.write(f'Max DBLO in Gouwens - {np.max(df[df["type"] == "Gouwens"].loc[:, "DBLO150 (mV)"])}\n')
f.write(f'Number of high DBLO Models in Gouwens - {len(df[df["type"] == "Gouwens"][df["DBLO150 (mV)"] > 14.3])}')
f.write(f'Number of moderate DBLO Models in Gouwens - {len(df[df["type"] == "Gouwens"][(df["DBLO150 (mV)"] <= 14.3) & (df["DBLO150 (mV)"] > 10)])}')
